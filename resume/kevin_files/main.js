...
function moveUpLayerHorizontal() {
    if ("horizontal" == layersMovement) {
        for (var e = 0; e < layerHorizontalArray.length; e++) {
            ...
            t <= -shiftUpLayerHorizontalDistance ? (t = -shiftUpLayerHorizontalDistance, layerHorizontalArray[e].style.top = t + "px", clearInterval(shiftUpLayerHorizontalTimer)) : layerHorizontalArray[e].style.top = t + "px",
...
function moveDownLayerHorizontal() {
    if ("horizontal" == layersMovement) {
        for (var e = 0; e < layerHorizontalArray.length; e++) {
            var t = layerHorizontalArray[e].offsetTop + shiftUpDownLayerHorizontalIncrement;
            0 <= t ? (t = 0, layerHorizontalArray[e].style.top = t + "px", clearInterval(shiftDownLayerHorizontalTimer)) : layerHorizontalArray[e].style.top = t + "px",
...
function orientRobby() {
    0 < deltaPageVerticalPosition && (robbyFramesDiv.style.top = "0px", robbyEyesCloseDiv.style.left = "90px"),
        deltaPageVerticalPosition < 0 && (robbyFramesDiv.style.top = "-200px", robbyEyesCloseDiv.style.left = "55px")
}
function storeDivs() {
...
function shiftRobbyFrame() {
    ...
    if (1 == isRobbySwimming && 1 == isRobbyBelowSeaLevel ? (robbyStartFrame = robbyStartSwimFrame, robbyStopFrame = robbyStopSwimFrame) : (robbyStartFrame = robbyStartRunFrame, robbyStopFrame = robbyStopRunFrame), robbyFramesDiv.style.left = -1 * robbyOneFrameWidth * (robbyStartFrame + counter) + "px", robbyStopFrame < robbyStartFrame + counter + switcher && (switcher *= -1), robbyStartFrame + counter + switcher == robbyStartFrame && (pageVerticalPositionWhenAnimateRobby1 = pageVerticalPosition), robbyStartFrame + counter + switcher < robbyStartFrame) {
        if (pageVerticalPositionWhenAnimateRobby1 == (pageVerticalPositionWhenAnimateRobby2 = pageVerticalPosition)) return clearShiftRobbyFrameTimer(),
...
function changeRobotHands() {
    if (robotHandChildrenLength <= changeRobotHandsCounter) changeRobotHandsCounter = 0,
        clearInterval(spinRobotHandsTimer),
...
function openSquidHands() {
    ...
    for (e = 0; e < squidHandCloseArray.length; e++) squidHandCloseArray[e].style.opacity = 0,
...
function closeSquidHands() {
    ...
    for (e = 0; e < squidHandCloseArray.length; e++) squidHandCloseArray[e].style.opacity = 1,
...
function rotateAlienHands() {
    (alienSteerPreviousAngle = alienSteerAngle) < (alienSteerAngle += alienSteerAngleIncrement) ? alienSteerAngleLimit < alienSteerAngle && (alienSteerAngleIncrement *= -1, alienSteerAngleLimit *= -1) : alienSteerAngle < alienSteerAngleLimit && (alienSteerAngleIncrement *= -1, alienSteerAngleLimit *= -1),
...
function blinkSeaAnimals(e) {
    for (var t = e,
        i = new Array,
        n = Math.ceil(5 * Math.random()), a = 0; a < n; a++) {
        var o = Math.floor(Math.random() * e.length);
        i.push(t[o])
    }
    for (a = 0; a < i.length; a++) $(i[a]).fadeTo(0, 1),
...
function positionContactConfirmationContainer() {
    for (var e = 0; e < contactConfirmationContainerArray.length; e++) contactConfirmationContainerArray[e].style.left = "not moving 1" == layersMovement || "not moving 2" == layersMovement ? robbyContainerDiv.offsetLeft + "px" : robbyMaxHorizontalDistance + "px",
...
// 页面插入两个按钮点击左边页面向上滚动1格，点击右边页面向下滚动1格
